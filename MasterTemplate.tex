...
\title{\heiti { 面向野外搜救的智能无人机系统 \\ \bfseries The Intelligent Unmanned Aircraft System for Wilderness Search and Rescue}}
\Etitle{A \LaTeX~Thesis Template for Wuhan University} % 英文题目
\author{武汉大学}
\StudentNumber{空中骑士队}           % 学号
\Eauthor{HUANG Zhenghua}                % 作者英文名
\Csupervisor{杨文\quad 张海剑}           % 指导教师中文名、职称
\Esupervisor{Prof.~HU Bao Qing}         % 指导教师英文名、职称
\Cmajor{王金旺\quad 余淮\quad 程文胜\quad 何婉君\quad 张恒}                        % 专业中文名[ SMD:专业类别（领域）]
\Emajor{Computational Mathematics}      % 专业英文名[ SMD:专业类别（领域）]
\Cspeciality{2017.04.30}                   % 研究方向
